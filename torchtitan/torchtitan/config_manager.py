...
class Metrics:
    ...
    """Whether to only evaluate the model and exit"""
    ...
    is_sweep: bool = False
    """Whether the job is a sweep"""

    sweep_id: str | None = None
    """The sweep id"""

    log_all_ranks: bool = False
    """Whether to log metrics to CLI for all ranks"""

    # sweep_config: dict = {}
    # """The sweep config"""

    wandb_project: str = "diloco"
    """The test project"""

    wandb_suffix: str = ""
    """The wandb suffix"""

    lm_evaluation_freq: int = 1000
    """How often to evaluate the model on the LM evaluation dataset, in iterations"""

    enable_lm_evaluation: bool = False
    """Whether to enable LM evaluation"""

    save_optimizer_delta: bool = False
    """Whether to save the optimizer delta"""

    optimizer_delta_path: str = "/checkpoint/optim/btherien/opt_ckpt_icml"
...
class Model:
    ...
    full_model_args : dict[Any, Any] | None = None
    """Full model args"""


@dataclass
class Optimizer:
    name: Literal["AdamW", "Adam", "DION", "GPA"] = "AdamW"
    """Optimizer to use (AdamW, Adam, or DION)"""

    gpa_kwargs: dict[Any, Any] | None = None
    """GPA kwargs"""

    ademamix_kwargs: dict[Any, Any] | None = None
    """AdEMAMix kwargs"""
...
class LRScheduler:
    ...
    warmup_min_factor: float | None = None
    """
    Minimum factor for warmup.
    """
...
class Training:
    ...
    eval_global_batch_size: int = 64
    """Evaluation global batch size"""
    ...
    eval_local_batch_size: int = 64
    """Evaluation local batch size"""
...
class Checkpoint:
    ...
    """
    Whether to delete checkpoints at the last step.
    """
    ...
    """
    The number of steps to offset the checkpointing interval.
    """

    keep_selected_checkpoints: list[int] = field(default_factory=list)
...
class FaultTolerance:
    ...
    """
    Whether to use continous centering.
    """

    backup_device: str | None = None
    """
    The device to use for the backup.
    """

    use_periodic_centering: bool = False
    """
    Whether to average the parameters.
    """
    ...
    """
    Whether to use SparseLoCo.
    """

    save_pseudograds: bool = False
    """
    Whether to save the pseudogradients.
    """

    use_quantized_outer_sgd: bool = False
    """
    Whether to use Quantized Outer SGD.
    """

    pseudograd_path: str = "pseudograds"
    """
    The path to save the pseudogradients.
    """

    module_fqns_per_model_fragment: list[list[str]] = field(default_factory=list)
    """
    The module names to fragment the model into.
    """
    ...
    fragment_kwargs: dict[str, Any] = field(default_factory=dict)
    """
    The keyword arguments to pass to the fragment.
    """
...
class JobConfig:
    ...
    outer_optimizer: OuterOptimizer = field(default_factory=OuterOptimizer)
    lr_scheduler: LRScheduler = field(default_factory=LRScheduler)
    data_train: DataTrain = field(default_factory=DataTrain)
    data_val: DataVal = field(default_factory=DataVal)
    ...
    def from_dict(cls, d: dict[str, Any]) -> "JobConfig":
        """
        Create a JobConfig instance from a dictionary.
        This recursively reconstructs nested dataclasses.
        """
        def _from_dict(dc_cls, dc_dict):
            if not hasattr(dc_cls, "__dataclass_fields__"):
                return dc_dict
            kwargs = {}
            for f in dc_cls.__dataclass_fields__:
                field_type = dc_cls.__dataclass_fields__[f].type
                if f in dc_dict:
                    value = dc_dict[f]
                    # If the field is a dataclass, recursively reconstruct
                    if hasattr(field_type, "__dataclass_fields__"):
                        kwargs[f] = _from_dict(field_type, value)
                    else:
                        kwargs[f] = value
            return dc_cls(**kwargs)
        return _from_dict(cls, d)

# Example usage: send a JobConfig to a dict and back again
if __name__ == "__main__":
    # Create a JobConfig instance with default values
    job_config = JobConfig()
    print("Original JobConfig:")
    print(job_config)
    ...
    print("\nJobConfig as dict:")
    import pprint
    pprint.pprint(job_config_dict)

    # Convert dict back to JobConfig
    job_config_restored = JobConfig.from_dict(job_config_dict)
    print("\nRestored JobConfig from dict:")
    print(job_config_restored)
...
class ConfigManager:
    ...
    def parse_args(self, args: list[str] = sys.argv[1:]) -> JobConfig:
        ...
        toml_values = self._maybe_load_mmengine_config(args)


        print("args before", args)
        # Filter args to only include --cfg_options and its values
        cfg_options_args, args = self._filter_cfg_options_args(args)

        print("cfg_options_args", cfg_options_args)
        print("args", args)

        config_cls = self._maybe_add_custom_args(args, toml_values)
        toml_values = self._maybe_add_custom_args_mmengine(cfg_options_args, toml_values)
    ...
    def _filter_cfg_options_args(self, args: list[str]) -> tuple[list[str], list[str]]:
        """
        Filter args to extract --cfg_options and return both the cfg_options args
        and the original args with cfg_options removed.
        
        Returns:
            tuple: (cfg_options_args, filtered_original_args)
        """
        cfg_options_args = []
        ...
        while i < len(args):
            if args[i] == '--cfg_options':
                cfg_options_args.append(args[i])
                i += 1
                # Add all following arguments until we hit another -- option or end
                while i < len(args) and not args[i].startswith('--'):
                    cfg_options_args.append(args[i])
                    i += 1
            else:
                filtered_original_args.append(args[i])
                i += 1
                
        return cfg_options_args, filtered_original_args

    def _maybe_add_custom_args_mmengine(
        self, args: list[str], cfg: Config
    ) -> Type[JobConfig]:    
        from mmengine.config import Config, DictAction
        import argparse
        ...
        if parsed_args.cfg_options:
            cfg.merge_from_dict(parsed_args.cfg_options)
        return cfg

    def _maybe_load_mmengine_config(self, args: list[str]) -> dict[str, Any] | None:
        # 1. Check CLI
