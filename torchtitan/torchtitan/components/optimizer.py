    ...
    if DION_AVAILABLE is None:
        try:
            ...
            from torchtitan.experiments.dion_optimizer.titan_dion import (
                DionOptimizersContainer, DionFTOptimizersContainer
            )
    ...
    if MUON_AVAILABLE is None:
        try:
            ...
            from torchtitan.experiments.dion_optimizer.titan_muon import (
                MuonOptimizersContainer, MuonFTOptimizersContainer
            )
...
class OptimizersContainer(Optimizer, Stateful, Generic[T]):
    ...
    **Limitations**
    ...
    def __init__(
        ...
        self.cache_state_dict: dict[str, Any] | None = None  # Cache for state_dict to avoid nested collectives
    ...
    def _get_state_dict_impl(self) -> dict[str, Any]:
        """Internal method that actually retrieves the state dict (involves distributed ops)."""
    ...
    def init_cache_state_dict(self) -> None:
        """Cache the optimizer state dict to avoid nested collective operations during dcp.save().
        
        This should be called before checkpointing to ensure get_optimizer_state_dict()
        (a distributed collective) is not called inside dcp.save() (another collective),
        which can cause deadlocks with FSDP.
        """
        self.cache_state_dict = self._get_state_dict_impl()

    def state_dict(self) -> dict[str, Any]:
        """Return cached state dict if available, otherwise compute it.
        
        When called inside dcp.save(), returning the cache avoids nested collectives.
        """
        if self.cache_state_dict is not None:
...
class FTOptimizersContainer(OptimizersContainer):
    def __init__(
        ...
        # Note: cache_state_dict is now initialized in parent class (OptimizersContainer)
    ...
    def init_cache_state_dict(self) -> None:
        # Use parent's implementation which caches _get_state_dict_impl()
        super().init_cache_state_dict()

    # state_dict() and load_state_dict() are now inherited from parent with caching support
...
def build_optimizers(
    ...
    **Note**
    ...
    ``OptimizersContainer``, and ``DionOptimizersContainer``.
    ...
    if name == "Dion":
        ...
        if ft_manager and ft_manager.enabled:
            return DionFTOptimizersContainer(
    ...
    if name == "Muon":
        ...
        muon_config = MuonOptimizerConfig(
            ...
            algorithm=optimizer_config.algorithm,
            cautious_wd=getattr(optimizer_config, "cautious_wd", False),
            use_polar_express=getattr(optimizer_config, "use_polar_express", False),
            # Parameter-specific optimizer selection
            scalar_optimizer=getattr(optimizer_config, "scalar_optimizer", "adamw"),
            embedding_optimizer=getattr(
                optimizer_config, "embedding_optimizer", "adamw"
        ...
        if ft_manager and ft_manager.enabled:
            return MuonFTOptimizersContainer(
                model_parts=model_parts,
    ...
    if name == "GPA":
        optimizer_kwargs = job_config.optimizer.gpa_kwargs
