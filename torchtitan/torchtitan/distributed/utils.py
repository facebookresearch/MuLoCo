...
def _get_reduce_op(reduceOp: str) -> dist.ReduceOp:
    """Convert string reduce op to dist.ReduceOp enum."""
    if reduceOp == "MAX":
        ...
        raise ValueError(f"Unknown reduce op: {reduceOp}")


def dist_reduce_async(
    x: torch.Tensor,
    reduceOp: str,
    mesh: DeviceMesh,
    extra_pg: dist.ProcessGroup | None = None,
) -> tuple[torch.Tensor, list[dist.Work]]:
    """Perform async distributed reduction on a tensor.

    Args:
        x (torch.Tensor): Input tensor.
        reduceOp (str): Reduce operation to perform ("MAX", "AVG", "SUM", "MIN").
        mesh (DeviceMesh): Device mesh to use for reduction.
        extra_pg (dist.ProcessGroup, optional): Extra process group to use for reduction.

    Returns:
        Tuple of (output tensor, list of async work handles to wait on).
    """
    if isinstance(x, DTensor):
        x = x.full_tensor()

    assert x.numel() == 1
    x = x.clone()  # Clone to avoid in-place modification of original
    reduce_op = _get_reduce_op(reduceOp)
    works = []

    if extra_pg is not None:
        work = dist.all_reduce(x, op=reduce_op, group=extra_pg, async_op=True)
        works.append(work)
        # Must wait for extra_pg reduction before mesh reduction
        work.wait()
        works = []  # Clear since we already waited

    work = dist.all_reduce(x, op=reduce_op, group=mesh.get_group(), async_op=True)
    works.append(work)

    return x, works


def dist_reduce_parallel(
    tensors: list[torch.Tensor],
    reduceOps: list[str],
    mesh: DeviceMesh,
    extra_pgs: list[dist.ProcessGroup | None],
) -> list[float]:
    """Perform multiple distributed reductions in parallel.

    This function launches all reductions asynchronously and waits for all
    to complete, which is more efficient than sequential reductions.

    Args:
        tensors: List of input tensors to reduce.
        reduceOps: List of reduce operations ("MAX", "AVG", "SUM", "MIN").
        mesh: Device mesh to use for reduction.
        extra_pgs: List of extra process groups (or None) for each reduction.

    Returns:
        List of reduced float values.
    """
    assert len(tensors) == len(reduceOps) == len(extra_pgs)

    # Separate into two groups: those with extra_pg and those without
    # Operations without extra_pg can run fully in parallel
    # Operations with extra_pg need their extra_pg reduction first
    ...
    for i, (t, op, pg) in enumerate(zip(tensors, reduceOps, extra_pgs)):
        if pg is None:
            indices_no_extra.append(i)
        else:
            indices_with_extra.append(i)

    # Phase 1: Launch extra_pg reductions for tensors that need it
    extra_pg_tensors = []
    extra_pg_works = []
    for i in indices_with_extra:
        t = tensors[i]
        if isinstance(t, DTensor):
            t = t.full_tensor()
        assert t.numel() == 1
        t = t.clone()
        extra_pg_tensors.append(t)
        reduce_op = _get_reduce_op(reduceOps[i])
        work = dist.all_reduce(t, op=reduce_op, group=extra_pgs[i], async_op=True)
        extra_pg_works.append(work)

    # Phase 2: Launch mesh reductions for tensors without extra_pg (can run in parallel)
    no_extra_tensors = []
    no_extra_works = []
    for i in indices_no_extra:
        t = tensors[i]
        if isinstance(t, DTensor):
            t = t.full_tensor()
        assert t.numel() == 1
        t = t.clone()
        no_extra_tensors.append(t)
        reduce_op = _get_reduce_op(reduceOps[i])
        work = dist.all_reduce(t, op=reduce_op, group=mesh.get_group(), async_op=True)
        no_extra_works.append(work)

    # Phase 3: Wait for extra_pg reductions and launch mesh reductions
    mesh_works_for_extra = []
    for work, t, i in zip(extra_pg_works, extra_pg_tensors, indices_with_extra):
        work.wait()
        reduce_op = _get_reduce_op(reduceOps[i])
        mesh_work = dist.all_reduce(t, op=reduce_op, group=mesh.get_group(), async_op=True)
        mesh_works_for_extra.append(mesh_work)

    # Phase 4: Wait for all mesh reductions
    for work in no_extra_works:
        work.wait()
    for work in mesh_works_for_extra:
        work.wait()

    # Collect results in original order
    results = [0.0] * len(tensors)
    for i, t in zip(indices_no_extra, no_extra_tensors):
        results[i] = t.item()
    for i, t in zip(indices_with_extra, extra_pg_tensors):
        results[i] = t.item()

    return results


def dist_broadcast_string(
    string: str,
    src_rank: int,
    current_rank: int,
    mesh: DeviceMesh,
    ...
    """Perform distributed broadcast of a string.

    Args:
        string (str): Input string to broadcast (only used on src_rank).
        src_rank (int): Source rank that has the string to broadcast.
        current_rank (int): Current rank that is broadcasting the string.
        mesh (DeviceMesh): Device mesh to use for broadcast.
        extra_pg (dist.ProcessGroup, optional): Extra process group to use for broadcast.
            Defaults to None. If provided, the broadcast will be called for the extra
            process group first, then for the mesh.
    
    Returns:
        str: The broadcasted string.
    """
    # Get current rank
    # current_rank = dist.get_rank()
    
    # Convert string to bytes and get length on source rank
    if current_rank == src_rank:
        string_bytes = string.encode('utf-8')
        string_len = torch.tensor([len(string_bytes)], dtype=torch.int64, device=device)
    else:
        string_len = torch.tensor([0], dtype=torch.int64, device=device)
    
    # Broadcast the length first
    if extra_pg is not None:
        dist.broadcast(string_len, src=src_rank, group=extra_pg)
    dist.broadcast(string_len, src=src_rank, group=mesh.get_group())
    
    # Create tensor for the string data
    if current_rank == src_rank:
        string_tensor = torch.tensor(list(string_bytes), dtype=torch.uint8, device=device)
    else:
        string_tensor = torch.zeros(string_len.item(), dtype=torch.uint8, device=device)
    
    # Broadcast the actual string data
    if extra_pg is not None:
        dist.broadcast(string_tensor, src=src_rank, group=extra_pg)
    dist.broadcast(string_tensor, src=src_rank, group=mesh.get_group())
    
    # Convert back to string
    return bytes(string_tensor.cpu().numpy()).decode('utf-8')
...
def init_distributed(job_config):
    ...
    if job_config.comm.trace_buf_size > 0:
        ...
        dump_dir = os.path.join(job_config.job.dump_folder, f"comm_trace/job_{os.environ['SLURM_JOB_ID']}")
