...
def create_parameter_groups(
    model_parts: List[nn.Module], dion_config, log_only_layer_zero: bool = True
) -> List[Dict[str, Any]]:
    ...
    Args:
        model_parts: List of model modules
        dion_config: Configuration object with optimizer settings
        log_only_layer_zero: If True, only log parameters from layer 0 (default: True)
    """
    param_groups = []
    ...
    for model in model_parts:
        ...
        is_layer_zero_param = lambda name: "layers.0" in name

        for name, param in model.named_parameters():
            if not param.requires_grad:
                continue

            # Determine if we should log this parameter based on layer filtering
            should_log = not log_only_layer_zero or is_layer_zero_param(name)
