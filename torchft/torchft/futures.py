    ...
    def _clear_del_queue(self) -> int:
        ...
        while True:
            try:
                ...
                #     == 2
                # ), f"items in del_queue reference should not have other references, found {refcount=}"
                # WORKAROUND for refcount bug - log warning instead of asserting
                # See: Job 1022950 crash analysis
                if refcount != 2:
                    import logging
                    logging.warning(
                        f"[torchft] del_queue refcount mismatch: expected 2, got {refcount}. "
                        f"This may indicate a memory leak but continuing anyway to avoid crash."
                    )
                del item
