...
class _StreamingDiLoCoFragment:
    ...
    def __init__(
        ...
        use_continuous_centering: bool = False,
        use_periodic_centering: bool = False,
        use_gpa: bool = False,
        use_sparseloco: bool = False,
        use_quantized_outer_sgd: bool = False,
        save_pseudograds: bool = False,
        pseudograd_path: str = "pseudograds",
        global_rank: int = 0,
        perturbation_std: float = 0.0,
        use_saved_perturbation: bool = False, 
        use_loss_weighted_pseudograd: bool = False,
        loss_decay_factor: float = 0.0,
        loss_weighting_temp: float = 10.0,
    ) -> None:
        ...
        self._use_gpa = use_gpa
        self._use_sparseloco = use_sparseloco
        self._use_periodic_centering = use_periodic_centering
        self._use_continuous_centering = use_continuous_centering
        self._outer_optimizer = outer_optimizer
        self._use_quantized_outer_sgd = use_quantized_outer_sgd
        self._pseudograd_path = pseudograd_path
        self._save_pseudograds = save_pseudograds
        self._global_rank = global_rank
        self._generator = None
        self._use_loss_weighted_pseudograd = use_loss_weighted_pseudograd
        self._loss_decay_factor = torch.tensor(loss_decay_factor,dtype=torch.float32)
        self._loss_weighting_temp = torch.tensor(loss_weighting_temp,dtype=torch.float32)
        if self._use_loss_weighted_pseudograd:
            self._loss_ema = torch.tensor(0.0,dtype=torch.float32)
        # Stores pending all reduce
        self._allreduce_futures: list[torch.futures.Future[torch.Tensor]] = []
        self._stream: Optional[torch.cuda.Stream] = (
            torch.cuda.Stream() if torch.cuda.is_available() else None
        )
        self.periodic_centering_lr = []
        self._perturbation_std = perturbation_std
        self._use_saved_perturbation = use_saved_perturbation
        ...
        if self._use_saved_perturbation:
            self._parameter_perturbation = {}
            self.save_parameter_perturbation()


    def update_loss_ema(self, loss : torch.Tensor) -> None:
        self._loss_ema = self._loss_ema.to(loss.device)
        self._loss_decay_factor = self._loss_decay_factor.to(loss.device)
        self._loss_ema = self._loss_ema * self._loss_decay_factor + loss * (1 - self._loss_decay_factor)


    def get_pseudograd_weighting(self, worker_losses: torch.Tensor) -> torch.Tensor:
        self._loss_weighting_temp = self._loss_weighting_temp.to(worker_losses.device)
        num_workers = worker_losses.shape[0]
        out = torch.softmax( (1 - worker_losses/worker_losses.sum()) / self._loss_weighting_temp, dim=0) * num_workers
        print(f"[rank {self._global_rank} loss_ema {self._loss_ema}]", worker_losses, out)
        return out[self._global_rank]


    def save_lr(self, lr : float) -> None:
        self.periodic_centering_lr.append(lr)
    ...
    def _save_grads(self) -> None:
        ...
        """
        if self._use_sparseloco or self._use_quantized_outer_sgd:
            # param_to_name = {f"{id(p)}": f"{name}" for name, p in self._model_fragment.named_parameters()}
            self._outer_optimizer._save_grads(
                grads=self._grads, 
                previous_params=self.original_parameters, 
                fragment=self._model_fragment
            )
            return
        ...
        if self._use_loss_weighted_pseudograd:        
            with (
                ...
                self.gathered_losses = self._manager.allgather(self._loss_ema).wait()
                self.wait()
                print(f"[rank {self._global_rank} gathered_losses]", self.gathered_losses)
            pgrad_weighting = self.get_pseudograd_weighting(torch.stack(self.gathered_losses))
        ...
        with torch.no_grad():
            for name, p in self._model_fragment.named_parameters():
                ...
                if self._use_gpa:
                    # all-reduce the z sequence
                    try:
                        state = self._local_optimizer.optimizers[0].state[p]
                        pseudogradient = state['z'].clone()
                    except KeyError as e:
                        print(e)
                        raise ValueError(f"Parameter {name} not found in GPA optimizer")
                elif self._use_continuous_centering or self._use_periodic_centering:
                    pseudogradient = local_param.clone()
                else:
                    ...
                    if self._use_saved_perturbation:
                        pseudogradient.sub_(self._parameter_perturbation[name].to(p.device))

                    if self._use_loss_weighted_pseudograd:
                        pseudogradient = pseudogradient * pgrad_weighting

                self._grads[name] = pseudogradient

        if self._save_pseudograds:
            os.makedirs(self._pseudograd_path, exist_ok=True)
            torch.save(self._grads, os.path.join(self._pseudograd_path, f"grads_{self._global_rank}.pt"))
            import time
            time.sleep(5*60)
            exit(0)
    ...
    @torch.profiler.record_function("torchft::local_sgd::save_average_parameters")
    def save_average_parameters(self) -> None:
        with torch.no_grad():
            # TODO: consider running copy on a separate stream
            for name, p in self._model_fragment.named_parameters():
                if self._use_gpa:
                    self._local_optimizer.optimizers[0].state[p]['z'] = self._grads[name]
                    del self._grads[name]
                else:
                    self.original_parameters[name] = self._grads[name]
                    del self._grads[name]
    ...
    def _merge_parameters(self) -> None:
        ...
        """
        assert not (self._use_continuous_centering \
            or self._use_gpa \
            or self._use_periodic_centering), "Merge parameters is not supported for continuous centering, GPA, and periodic centering"
    ...
    def get_generator(self,device: torch.device) -> torch.Generator:
        ...
        self._generator.manual_seed(self._global_rank * 42)
        return self._generator
    ...
    def save_parameter_perturbation(self) -> None:
        """
        Saves the perturbed parameters.
        """
        
        for name, p in self._model_fragment.named_parameters():
            self.get_generator(p.device)
            if isinstance(p, DTensor):
                self._parameter_perturbation[name] = DTensor.from_local(
                    torch.randn(p.shape, dtype=p.dtype, generator=self._generator) * self._perturbation_std,
                    p.device_mesh,
                    p.placements,
                    shape=p.shape,
                    stride=p.stride(),
                ...
                self._parameter_perturbation[name] = torch.randn(p.shape, device=p.device, dtype=p.dtype, generator=self._generator) * self._perturbation_std
                # if len(p.shape) == 1:
                #     print(name, self._parameter_perturbation[name][:10])
                # elif len(p.shape) == 2:
                #     print(name, self._parameter_perturbation[name][:10,:10])
                # exit(0)

    def _perturb_parameters(self) -> None:
        """
        Merges the local and global parameters.
        """
        if self._perturbation_std == 0.0:
            return
        ...
        if self._use_saved_perturbation:
            for name, p in self._model_fragment.named_parameters():
                p.data.add_(self._parameter_perturbation[name])
        else:
            for name, p in self._model_fragment.named_parameters():
                # we averaged the local version of the tensor so need to copy it back as a DTensor
                if isinstance(p, DTensor):
                    p.data.add_(
                        DTensor.from_local(
                            torch.randn(p.shape, dtype=p.dtype) * self._perturbation_std,
                            p.device_mesh,
                            p.placements,
                            shape=p.shape,
                            stride=p.stride(),
                    ...
                    p.data.add_(torch.randn(p.shape, device=p.device, dtype=p.dtype)*self._perturbation_std)


    @torch.profiler.record_function("torchft::local_sgd::recenter_parameters")
    def recenter_parameters(self, lr : float) -> None:
        """
        Recenter the parameters around the global parameters.
        """
        if self._use_periodic_centering:
            lr = torch.tensor(self.periodic_centering_lr).sum()
            self.periodic_centering_lr = []
        ...
        for name, p in self._model_fragment.named_parameters():
            if isinstance(p, DTensor):
                if self._use_periodic_centering:
                    lr = lr.to(p.device)
                p.data.lerp_(DTensor.from_local(
                    self._local_parameters[name],
                    p.device_mesh,
                    p.placements,
                    shape=p.shape,
                    stride=p.stride(),
                ), self._fragment_update_alpha * lr)
            else:
                if self._use_periodic_centering:
                    lr = lr.to(p.device)
                p.data.lerp_(self.original_parameters[name], self._fragment_update_alpha * lr)
    ...
    def prepare_sync(self) -> None:
        ...
        with (
            ...
            if self._use_sparseloco or self._use_quantized_outer_sgd:
                self._allgather_grads()
                for name, work in self._allreduce_futures:
                    self._grads[name] = work.wait()
            else:
                self._average_grads()
    ...
    def perform_sync(self) -> bool:
        ...
        if not ( self._use_continuous_centering \
            or self._use_gpa \
            or self._use_periodic_centering):
            self._save_local_parameters()
        # Restore the parameters back to the previous state
        # after this the local parameters no longer exist...
        # instead what we want is to keep a separate copy of the local parameters.
        ...
        if should_commit:
            ...
            if self._use_gpa:
                self.save_average_parameters()
                for opt in self._local_optimizer.optimizers:
                    opt.post_communication()
            elif self._use_periodic_centering:
                self.save_average_parameters()
                # we must immediately recenter the parameters
                # when using periodic centering
                self.recenter_parameters(lr=1.0)
            elif self._use_continuous_centering:
                # for continuous centering, we recenter parameters
                # during the training loop
                self.save_average_parameters()
            elif self._use_sparseloco or self._use_quantized_outer_sgd:
                self._outer_optimizer._set_grads(self._grads, self._model_fragment, self._global_step)
        ...
        # reset the loss ema to zero to avoid bias from previous steps
        if self._use_loss_weighted_pseudograd:
            self._loss_ema = torch.tensor(0.0,dtype=torch.float32)

        # for i, (name, p) in enumerate(self._model_fragment.named_parameters()):
        #     if p.grad is not None:
        #         print(f"Gradient for {name}: sum={p.grad.sum()}")
        #     else:
    ...
    def _allgather_grads(self) -> None:
        """
        Allgather the gradients across the manager group.
        """
        # if self.use_bucketization:
        #   raise NotImplementedError("Bucketized allgather is not implemented")
        # else:
        self._allgather_per_param()

    def _allgather_per_param(self) -> None:
        """Performs allreduce on each gradient tensor separately (original method)."""
        for name, p in self._model_fragment.named_parameters():
            # Perform allreduce on the pseudogradients

            if self._use_quantized_outer_sgd:
                key = name + '_metadata'
                work = self._manager.allgather(
                    self._grads[key]
                )
                self._allreduce_futures.append((key, work))
                ...
                if self._outer_optimizer.compressor.use_quantization:
                    key = name + '_quant_shift_lookup'
                    work = self._manager.allgather(
                        self._grads[key]
                    )
                    self._allreduce_futures.append((key, work))
...
class DiLoCo:
    ...
    def __init__(
        ...
        self._fragments: List[_StreamingDiLoCoFragment] = [
            _StreamingDiLoCoFragment(
                ...
                use_continuous_centering,
                use_periodic_centering,
                use_gpa,
                use_sparseloco,
                use_quantized_outer_sgd,
                save_pseudograds,
                pseudograd_path,
                global_rank,
                **fragment_kwargs
    ...
    def update_loss_ema(self, loss : torch.Tensor) -> None:
        for fragment in self._fragments:
            fragment.update_loss_ema(loss)

    def _save_lr_to_fragments(self, lr : float) -> None:
        for fragment in self._fragments:
            fragment.save_lr(lr)
    ...
    def recenter_parameters(self, lr : float) -> None:
        ...
        """
        for fragment in self._fragments:
            fragment.recenter_parameters(lr)
    ...
    def _step_post_hook(
        ...
        for i, fragment in enumerate(self._fragments):
            if not fragment.should_prepare_fragment(step):
                # skip if we are not at the step to sync the fragment
                continue
        ...
        for i, fragment in enumerate(self._fragments):
            ...
            # `max_step` will have the same `local_step`. That isq
