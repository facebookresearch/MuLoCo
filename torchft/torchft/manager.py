...
class Manager:
    ...
    @torch.profiler.record_function("torchft::manager::allgather")
    def allgather(
        self, tensor: torch.Tensor,
    ) -> torch.futures.Future[List[torch.Tensor]]:
        """
        Fault tolerant allgather the tensor and return a Future that will be completed when
        the tensor is ready.

        If an error occurs during the allgather:
        ...
        * The tensor must be zeroed before being used as it may be corrupted.
        ...
        Args:
            tensor: the tensor to allgather
        Returns:
            a Future that will be completed with the allgathered tensors
        """
        if self.errored():
            fut = torch.futures.Future()  # pyre-fixme[29]: not a function
            # Return a list with just the input tensor for consistency
            fut.set_result([tensor])
            return fut
        ...
        # TODO: increase timeout when waiting when healing
        try:
            # Run the allgather async and save the work object so we can wait on
            # it later.
            ws = num_participants
            ...
            opts.asyncOp = True
            work = self._pg.allgather([tensor_list], [tensor], opts)
            fut = work.get_future()
            ...
            # schedule processing as a continuation
            # on the Future
            @torch.profiler.record_function("torchft::manager::allgather::callback")
            def callback(
                fut: torch.futures.Future[List[torch.Tensor]],
            ) -> List[torch.Tensor]:
                nonlocal tensor_list, stream

                # change the stream to avoid making the callback stream
                # dependent on process group stream running the allgather
                with torch.cuda.stream(stream) if stream is not None else nullcontext():
                    fut.value()
                    return tensor_list
